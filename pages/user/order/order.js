...
Page({
  ...
  getData() {
    let that = this;
    RQ.postRequest("/api/user/getOrderList", { token: wx.getStorageSync('token'), type: that.data.type }, function (res) {
      if (res.status == 1) {
        let list = res.result;
        if(list.length > 0) {
          that.setData({
        ...
        for(let i = 0; i < list.length; i++) {
          ...
          if (list[i].order_status_code == "WAITCCOMMENT") {
  ...
  sureReceive(event) {
    let that = this;
    RQ.postRequest("/api/user/orderConfirm", { token: wx.getStorageSync('token'), order_id: event.currentTarget.dataset.id }, function (res) {
      if (res.status == 1) {
        wx.showToast({
          title: '确认成功',
          icon: 'success'
