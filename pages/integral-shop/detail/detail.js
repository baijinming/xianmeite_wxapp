...
Page({
  ...
  getDetail: function () {
    request.postRequest('/api/Integral/goodsInfo', { goods_id: this.data.goods_id}, (res) => {
      let result = res.result;
      var article = result.content.split('<style>')[1].split('</style>')[1]
      WxParse.wxParse('article', 'html', article, this, 5);
      let gallery = [{image_url:result.original_img}] ;
      this.setData({
        goods: result,
        gallery: gallery,
      })
