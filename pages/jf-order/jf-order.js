...
Page({
  ...
  getData() {
    let that = this;
    RQ.postRequest("/api/user/getIntegralOrderList", { token: wx.getStorageSync('token'), type: that.data.type }, function (res) {
      if (res.status == 1) {
        let list = res.result;
        if (list.length > 0) {
          that.setData({
  ...
  sureReceive(event) {
    let that = this;
    RQ.postRequest("/api/user/orderIntegralConfirm", { token: wx.getStorageSync('token'), order_id: event.currentTarget.dataset.id }, function (res) {
      if (res.status == 1) {
        wx.showToast({
          title: '确认成功',
          icon: 'success'
